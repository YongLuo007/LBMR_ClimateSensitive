...
doEvent.LBMR_GMOrig = function(sim, eventTime, eventType, debug = FALSE) {
  if (eventType == "init") {
    sim <- scheduleEvent(sim, start(sim) + params(sim)$LBMR_GMOrig$growthInitialTime,
                         "LBMR_GMOrig", "mortalityAndGrowth", eventPriority = 5)
  } else if (eventType == "mortalityAndGrowth") {
    sim <- sim$LBMR_GMOrigMortalityAndGrowth(sim)
    sim <- scheduleEvent(sim, time(sim) + 1, "LBMR_GMOrig",
...
LBMR_GMOrigInit <- function(sim) {
  return(invisible(sim))
}

### template for your event1
LBMR_GMOrigMortalityAndGrowth <- function(sim) {
