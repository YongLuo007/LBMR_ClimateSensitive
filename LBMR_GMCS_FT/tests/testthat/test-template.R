...
#    `functionName` is a function's name in your module (e.g., `LBMR_GMCS_FTEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/LBMR_ClimateSensitive/LBMR_GMCS_FT/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LBMR_GMCS_FT")
  ...
  parameters <- list(
    ...
    LBMR_GMCS_FT = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LBMR_GMCS_FTEvent1", envir = .GlobalEnv)) {
    simOutput <- LBMR_GMCS_FTEvent1(mySim)
  } else {
    simOutput <- mySim$LBMR_GMCS_FTEvent1(mySim)
  }
  ...
  if (exists("LBMR_GMCS_FTEvent2", envir = .GlobalEnv)) {
    simOutput <- LBMR_GMCS_FTEvent2(mySim)
  } else {
    simOutput <- mySim$LBMR_GMCS_FTEvent2(mySim)
  }
