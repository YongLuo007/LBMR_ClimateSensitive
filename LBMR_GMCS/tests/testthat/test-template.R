...
#    `functionName` is a function's name in your module (e.g., `LBMR_GMCSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/LBMR_ClimateSensitive/LBMR_GMCS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LBMR_GMCS")
  ...
  parameters <- list(
    ...
    LBMR_GMCS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LBMR_GMCSEvent1", envir = .GlobalEnv)) {
    simOutput <- LBMR_GMCSEvent1(mySim)
  } else {
    simOutput <- mySim$LBMR_GMCSEvent1(mySim)
  }
  ...
  if (exists("LBMR_GMCSEvent2", envir = .GlobalEnv)) {
    simOutput <- LBMR_GMCSEvent2(mySim)
  } else {
    simOutput <- mySim$LBMR_GMCSEvent2(mySim)
  }
